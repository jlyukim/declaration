...
export function getSetStrFromCard(cardValue: string) {
  if (cardValue.includes("joker")) {
...
export function getSetCardFromCard(cardValue: string) {
  const set = getSetFromCard(cardValue, sets);
  if (set) {
    return set[5]; // Return the last card in the set
  }
  return "cardback"; // Default card if no set found
}
